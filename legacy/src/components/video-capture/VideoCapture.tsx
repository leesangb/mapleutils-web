...
import NotVisibleImage from './NotVisibleImage';
import NotVisibleVideo from './NotVisibleVideo';
...
const VideoCapture = (props: PropsWithChildren<VideoCaptureProps>) => {
    ...
    useEffect(() => {
        const settings = LocalStorageHelper.load<Partial<{
            x: number,
            y: number,
            ratio: number,
            showJump: boolean
        }> | null>(LocalStorageKey.VIDEO_CAPTURE_SETTINGS);
        if (settings) {
            dispatch(settings);
        }
    }, []);
